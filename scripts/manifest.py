...
canonical `worlds/<apworld>.json` with two fields appended:

    `igdb_id`         — IGDB game id (int) used by the Karen/IGDB-game-details Action
...
apworld. When `--igdb-id` is omitted and the source manifest already contains one,
it is preserved.
"""
...
DEFAULT_MODULE_LOCATION_TEMPLATE = (
    "https://github.com/MultiworldGG/MultiworldGG/tree/worlds-mirror/worlds/{apworld}"
)
...
def add_world_metadata(
    ...
    *,
    ...
    with the apworld. `igdb_id` is written only when supplied AND not already present
    ...
    if module_location is None:
        module_location = module_location_template.format(apworld=apworld)
...
def write_world_manifest(manifest: dict, apworld: str, output_dir: Path) -> Path:
    """Write `<output_dir>/<apworld>.json` with canonical formatting (4-space indent, trailing newline).
    ...
    target = output_dir / f"{apworld}.json"
...
def _cli(argv: Optional[list[str]] = None) -> int:
    parser = argparse.ArgumentParser(
        description="Build a worlds/<apworld>.json manifest from a per-world archipelago.json.",
    )
    parser.add_argument("--apworld", required=True, help="World apworld (e.g. 'oot')")
    parser.add_argument(
        "--archipelago-json",
        required=True,
        type=Path,
    ...
    target = write_world_manifest(manifest, args.apworld, args.output_dir)
